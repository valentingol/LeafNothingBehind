"""Training functions for Vanadium."""
import argparse
import os
...
import wandb
from lnb.architecture.models import Vanadium
...
def train_val_loop(
    ...
    os.makedirs(f"../models/vanadium/{run_id}", exist_ok=True)
    with open(
        f"../models/vanadium/{run_id}/config.yaml", "w", encoding="utf-8",
    ) as cfg_file:
    ...
    for epoch in range(n_epochs):
        ...
        if (epoch + 1) % train_config["save_interval"] == 0:
            torch.save(
                model.state_dict(),
                f"../models/vanadium/{run_id}/{run_id}_ep{epoch + 1}.pth",
            )
            print(
                f"Model saved to ../models/vanadium/{run_id}/"
                f"{run_id}_ep{epoch + 1}.pth",
            )

    # Save final model
    torch.save(model.state_dict(), f"../models/vanadium/{run_id}/{run_id}_last.pth")
    print(f"Model saved to ../models/vanadium/{run_id}/{run_id}_last.pth")


def run(config: Dict) -> None:
    """Run training."""
    model = Vanadium(config["model"])
    train_dataloader = DataLoader(
...
def main() -> None:
    ...
    parser.add_argument(
        "--config_path", type=str, required=False, default="config/vanadium/base.yaml",
    )
    ...
    run_id = max(int(name) for name in os.listdir("../models/vanadium")) + 1
    config["run_id"] = run_id
    wandb.init(
        project="lnb", entity="leaf_nothing_behind", group="vanadium", config=config,
    )
