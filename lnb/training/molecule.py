"""Training functions for Strontium."""
import argparse
import os
...
import wandb
from lnb.architecture.cloud_models import Molecule
...
def train_val_loop(
    ...
    os.makedirs(f"../models/cloud_strontium/{run_id}", exist_ok=True)
    with open(
        f"../models/cloud_strontium/{run_id}/config.yaml", "w", encoding="utf-8",
    ) as cfg_file:
    ...
    for epoch in range(n_epochs):
        ...
        if (epoch + 1) % train_config["save_interval"] == 0:
            torch.save(
                model.state_dict(),
                f"../models/cloud_strontium/{run_id}/{run_id}_ep{epoch + 1}.pth",
            )
            print(
                f"Model saved to ../models/cloud_strontium/{run_id}/"
                f"{run_id}_ep{epoch + 1}.pth",
            )

    # Save final model
    torch.save(model.state_dict(),
               f"../models/cloud_strontium/{run_id}/{run_id}_last.pth")
    print(f"Model saved to ../models/cloud_strontium/{run_id}/{run_id}_last.pth")
...
def run(config: Dict) -> None:
    ...
    base_model = Strontium(config["model"])
    base_model.load_state_dict(
        torch.load("../models/strontium/16062004/16062004_last.pth"),
    )
    cloud_model = Strontium(config["model"])
    cloud_model.load_state_dict(
        torch.load("../models/cloud_strontium/2/2_last.pth"),
    )
    model = Molecule(base_model, cloud_model).to(device)
    train_dataloader = DataLoader(
    ...
    val_loader_config["batch_size"] = 1  # Hard-coded batch size for validation
    val_dataloaders = []
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        default="config/strontium/exp.yaml",
    )
    ...
    run_id = max(int(name) for name in os.listdir("../models/cloud_strontium")) + 1
    config["run_id"] = run_id
    wandb.init(
        project="lnb", entity="leaf_nothing_behind",
        group="molecule", config=config,
    )
