"""Training functions for resunet."""
import argparse
import os
...
import wandb
from lnb.architecture.resunet import ExtremeResUnet
...
def train_val_loop(
    ...
    os.makedirs(f"../models/resunet/{run_id}", exist_ok=True)
    with open(
        f"../models/resunet/{run_id}/config.yaml", "w", encoding="utf-8"
    ) as cfg_file:
    ...
    for epoch in range(n_epochs):
        ...
        if (epoch + 1) % train_config["save_interval"] == 0:
            torch.save(
                model.state_dict(),
                f"../models/resunet/{run_id}/{run_id}_ep{epoch + 1}.pth",
            )
            print(
                f"Model saved to ../models/resunet/{run_id}/"
                f"{run_id}_ep{epoch + 1}.pth"
            )

    # Save final model
    torch.save(model.state_dict(), f"../models/resunet/{run_id}/{run_id}_last.pth")
    print(f"Model saved to ../models/resunet/{run_id}/{run_id}_last.pth")


def run(config: Dict) -> None:
    """Run training."""
    model = ExtremeResUnet(**config["model"])
    train_dataloader = DataLoader(
...
def main() -> None:
    ...
    parser.add_argument(
        "--config_path", type=str, required=False, default="config/resunet/base.yaml"
    )
    ...
    wandb.init(
        project="lnb", entity="leaf_nothing_behind", group="resunet", config=config
    )
