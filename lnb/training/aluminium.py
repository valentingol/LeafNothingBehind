"""Training functions for Aluminium."""
import argparse
import os
...
import wandb
from lnb.architecture.models import Aluminium
...
def train_val_loop(config: Dict, model: nn.Module, train_dataloader: DataLoader,
    ...
    os.makedirs(f'../models/aluminium/{run_id}', exist_ok=True)
    with open(f'../models/aluminium/{run_id}/config.yaml',
    ...
    for epoch in range(n_epochs):
        ...
        if (epoch + 1) % train_config['save_interval'] == 0:
            torch.save(model.state_dict(),
                       f'../models/aluminium/{run_id}/{run_id}_ep{epoch + 1}.pth')
            print(f'Model saved to ../models/aluminium/{run_id}/'
                  f'{run_id}_ep{epoch + 1}.pth')

    # Save final model
    torch.save(model.state_dict(), f'../models/aluminium/{run_id}/{run_id}_last.pth')
    print(f'Model saved to ../models/aluminium/{run_id}/{run_id}_last.pth')


def run(config: Dict) -> None:
    """Run training."""
    model = Aluminium(config['model'])
...
def main() -> None:
    ...
    parser.add_argument('--config_path', type=str, required=False,
                        default='config/aluminium/base.yaml')
    ...
    wandb.init(project='lnb', entity='leaf_nothing_behind', group='aluminium',
               config=config)
