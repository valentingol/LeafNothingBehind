...
def train_val_loop(
    ...
    with open(
        f"../models/manganese/{run_id}/config.yaml", "w", encoding="utf-8",
    ) as cfg_file:
    ...
    for epoch in range(n_epochs):
        ...
        if (epoch + 1) % train_config["save_interval"] == 0:
            ...
            print(
                f"Model saved to ../models/manganese/{run_id}/"
...
def main() -> None:
    ...
    parser.add_argument(
        "--config_path", type=str, required=False, default="config/manganese/base.yaml",
    )
    ...
    wandb.init(
        project="lnb", entity="leaf_nothing_behind", group="manganese", config=config,
    )
