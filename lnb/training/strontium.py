...
def train_val_loop(config: Dict, model: nn.Module, train_dataloader: DataLoader,
    ...
    with open(f'../models/strontium/{run_id}/config.yaml',
    ...
    for epoch in range(n_epochs):
        ...
        if (epoch + 1) % train_config['save_interval'] == 0:
            torch.save(model.state_dict(),
                       f'../models/strontium/{run_id}/{run_id}_ep{epoch + 1}.pth')
            print(f'Model saved to ../models/strontium/{run_id}/'
...
def main() -> None:
    ...
    parser.add_argument('--config_path', type=str, required=False,
                        default='config/strontium/base.yaml')
    ...
    wandb.init(project='lnb', entity='leaf_nothing_behind', group='strontium',
               config=config)
