"""Training functions for hassium."""
import argparse
import os
...
import wandb
from lnb.architecture.models import Hassium
...
def parse_data_device(
    data: torch.Tensor, glob: torch.Tensor, weather_data: torch.Tensor, device: torch.device,
) -> ParsedDataType:
...
def train_val_loop(
    ...
    os.makedirs(f"../models/hassium/{run_id}", exist_ok=True)
    with open(
        f"../models/hassium/{run_id}/config.yaml", "w", encoding="utf-8",
    ) as cfg_file:
    ...
    for epoch in range(n_epochs):
        ...
        for data, glob, weather_data in train_dataloader:
            i_batch += 1
        ...
        if (epoch + 1) % train_config["save_interval"] == 0:
            torch.save(
                model.state_dict(),
                f"../models/hassium/{run_id}/{run_id}_ep{epoch + 1}.pth",
            )
            print(
                f"Model saved to ../models/hassium/{run_id}/"
        ...
        if mean_valid_loss < best_val_loss:
            ...
            torch.save(
                model.state_dict(),
                f"../models/hassium/{run_id}/{run_id}_best.pth",
            )
            print(
                f"Model saved to ../models/hassium/{run_id}/"
                f"{run_id}_best.pth",
    ...
    # Save final model
    torch.save(model.state_dict(), f"../models/hassium/{run_id}/{run_id}_last.pth")
    print(f"Model saved to ../models/hassium/{run_id}/{run_id}_last.pth")


def run(config: Dict) -> None:
    """Run training."""
    model = Hassium(config["model"])
    train_dataloader = DataLoader(
...
def main() -> None:
    ...
    parser.add_argument(
        "--config_path", type=str, required=False, default="config/hassium/base.yaml",
    )
    ...
    if len(os.listdir("../models/hassium")) == 0:
        run_id = 0
    else:
        run_id = max(int(name) for name in os.listdir("../models/hassium")) + 1
    config["run_id"] = run_id
    wandb.init(
        project="lnb", entity="leaf_nothing_behind", group="hassium", config=config,
    )
