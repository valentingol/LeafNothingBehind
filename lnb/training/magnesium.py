...
def train_val_loop(config: Dict, model: nn.Module, train_dataloader: DataLoader,
    ...
    with open(f'../models/magnesium/{run_id}/config.yaml',
    ...
    for epoch in range(n_epochs):
        ...
        if (epoch + 1) % train_config['save_interval'] == 0:
            ...
            print(f'Model saved to ../models/magnesium/{run_id}/'
...
def main() -> None:
    ...
    parser.add_argument('--config_path', type=str, required=False,
                        default='config/magnesium/base.yaml')
    ...
    wandb.init(project='lnb', entity='leaf_nothing_behind', group='magnesium',
               config=config)
