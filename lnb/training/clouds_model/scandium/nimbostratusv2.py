...
import wandb
from lnb.architecture.cloud_models import Nimbostratusv2
...
def run(config: Dict) -> None:
    ...
    # Cloud model
    model = Nimbostratusv2(base_model=base_model, model_config=config["model"])
    model = model.to(device)
...
def main() -> None:
    ...
    run_id = max(int(name) for name in os.listdir(
        "../models/scandium_nimbostratus") if not name.startswith('.')) + 1
