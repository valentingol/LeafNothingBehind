...
import wandb
from lnb.architecture.cloud_models import Cirrus
...
def run(config: Dict) -> None:
    ...
    # Cloud model
    model = Cirrus(base_model=base_model, model_config=config["model"])
    model = model.to(device)
...
def main() -> None:
    ...
    run_id = max(int(name) for name in os.listdir(
        "../models/scandium_cirrus") if not name.startswith('.')) + 1
    config["run_id"] = run_id
    wandb.init(
        project="lnb", entity="leaf_nothing_behind", group="scandium_cirrus", config=config
    )
