...
from lnb.architecture.modules import AutoEncoder, InceptionBlock, SegFormerTransposed
...
class Scandium(Atom):
    ...
    def __init__(self, model_config: Dict) -> None:
        ...
        for i in range(len(glob_module_dims) - 1):
            ...
            if i < len(glob_module_dims) - 2:
                self.conv_glob.add_module(f"glob_conv_{i+1}_relu", nn.ReLU())
        # Convolutional block
        first_dim = (
            mask_out_dim * 2 + glob_module_dims[-1] + s1_ae_config["out_dim"] * 3 + 2
        )
        conv_block_dims = [first_dim] + conv_block_dims
        self.conv_block = nn.Sequential()
        for i in range(len(conv_block_dims) - 1):
            in_channels = conv_block_dims[i] if i == 0 else conv_block_dims[i] + 2
            ...
            self.conv_block.add_module(f"conv_{i+1}_relu", nn.ReLU())
        # Last convolutional layer
        self.last_conv = nn.Sequential()
        self.last_conv.add_module(
            f"conv_{len(conv_block_dims)}",
            nn.Conv2d(conv_block_dims[-1] + 2, 1, kernel_size=1, stride=1, padding=0),
        )
    ...
    def forward(
        ...
        t_input = torch.cat(
            [in_lai, mask_lai_embed, s1_input],
            dim=1,
        )  # (batch, c, h, w)
...
class Vanadium(Atom):
    ...
    def __init__(self, model_config: Dict) -> None:
        ...
        self.segformer = SegFormerTransposed(
...
class Manganese(Atom):
    ...
    module_config: Dict
        Model configuration.
            ...
            glob_module_dims : List[int]
                Channels of the global features module.
            conv_block_dims : List[int]
                Channels of the inception convolutional block.
            conv_block_dropout : float
                Dropout rate of the inception convolutional block.
    """
    ...
    def __init__(self, model_config: Dict) -> None:
        ...
        glob_module_dims = model_config["glob_module_dims"]
        conv_block_dims = model_config["conv_block_dims"]
        conv_block_dropout = model_config["conv_block_dropout"]
        # AE for Sentinel-1 data
        self.s1_ae = AutoEncoder(**s1_ae_config)
        # Convolutional layers for LAI mask
        self.conv_lai_mask = nn.Conv2d(
...
class Strontium(Atom):
    """Strontium model for LNB."""

    def __init__(self, model_config: Dict) -> None:
        super().__init__(model_config)
        if model_config["sodium_config"]["pretrained"]:
            self.sodium = self._load_model(
                "sodium",
                model_config["sodium_config"]["run_id"],
            )
        else:
        ...
        if model_config["aluminium_config"]["pretrained"]:
            self.aluminium = self._load_model(
                "aluminium",
                model_config["aluminium_config"]["run_id"],
            )
        else:
    ...
    def _load_model(
        ...
        with open(
            os.path.join(model_folder, "config.yaml"),
            encoding="utf-8",
        ) as cfg_file:
            model_config = yaml.safe_load(cfg_file)["model"]

        if model_type == "sodium":
            model: Atom = Sodium(model_config)
